...
priority: P2
milestone: post-mvp
source: PR manavsehgal/stagent#6 + .claude/skills/architect/references/tdr-031-workflow-status-response-contract.md + features/architect-report.md
...
As a developer adding a new workflow pattern to Stagent, I want pattern-specific rendering to live in pattern-specific components with a type-enforced response contract, so that I cannot accidentally introduce crashes by adding fields to a shared `data.steps[]` shape and forgetting that one pattern's arm of the union does not have them.
...
- [ ] `features/changelog.md` has a "Fixed" entry citing PR manavsehgal/stagent#6 and a "Hardened" entry for the router refactor, both under the implementation date.
...
- **Source:** PR manavsehgal/stagent#6 (`fix/workflow-loop-status-crash`) — the symptom fix this spec supersedes.
