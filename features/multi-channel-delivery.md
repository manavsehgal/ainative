...
Add Slack and Telegram as outbound delivery channels for agent results, heartbeat notifications, workflow completions, and approval requests. Phase 1 is delivery-only (Stagent pushes to channels). Phase 2 (bidirectional interaction — users reply in Slack/Telegram to trigger agent actions) is deferred.
...
As a solo founder, I want my AI agents to send me results, alerts, and heartbeat summaries in Slack or Telegram, so that I stay informed without needing to check the Stagent dashboard.
...
src/lib/channels/
  ...
  formatter.ts       -- Format Stagent outputs into channel-appropriate messages
```
...
4. **Approval requests** (in notification system): When a canUseTool gate fires and requires human approval, push an approval notification to configured channels with a link back to the Stagent inbox.

### Message Formatting

`formatter.ts` converts Stagent's internal result format to channel-appropriate messages:
...
- **Approval request:** "Approval needed: [profile name] wants to use [tool]" + "Approve in Stagent" link
...
- [ ] Approval requests are pushed with a link back to the Stagent inbox
...
- Source: `ideas/vision/Stagent-OpenClaw-Companion-Research-Report.md` — Section 3.4 (Multi-Channel Delivery)
