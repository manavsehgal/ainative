...
As a Stagent developer, I want a Supabase backend configured with all tables, RLS, and Edge Functions so that the license, sync, telemetry, and marketplace systems have a cloud foundation.

## Technical Approach

- Create Supabase project `stagent-cloud`, configure Auth (email/password + GitHub OAuth)
...
- Storage bucket: `stagent-sync` with per-user path RLS
...
- [ ] Storage bucket `stagent-sync` has per-user path policy
