...
describe("scanBundleSection<T>", () => {
  ...
  it("skips non-yaml files", () => {
    ...
    const calls: string[] = [];
    scanBundleSection({ rootDir: tmp, section: "profiles", parseFile: (f: string) => { calls.push(path.basename(f)); return null; } });
  ...
  it("drops entries where parseFile returned null", () => {
    ...
    const out = scanBundleSection<number>({ rootDir: tmp, section: "tables", parseFile: (f: string) => path.basename(f) === "a.yaml" ? 1 : null });
  ...
  it("swallows parseFile throws with a warning, continues other files", () => {
    ...
    const out = scanBundleSection<string>({
      ...
      parseFile: (f: string) => { if (path.basename(f) === "bad.yaml") throw new Error("boom"); return "ok"; },
    });
