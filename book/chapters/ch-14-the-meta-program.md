...
Chapter 11 described the machine that builds machines. Chapter 13 showed what happens when that machine builds a wealth management application in a single day. This chapter is about what happens next — the step that transforms Stagent from a product into a platform, and the pattern that makes "build your own domain application" a configuration exercise instead of a software project.

The pattern is deceptively simple. Clone the Stagent repository into a sibling directory. Point a new `STAGENT_DATA_DIR` at an isolated SQLite instance so the clone cannot collide with your main workspace. Create a long-lived branch — `wealth-mgr`, `growth-mgr`, whatever domain you are shaping. Then prompt Stagent — or use its UI — to program a domain-specific application using Stagent's own primitives: tables, profiles, workflow blueprints, schedules, pages, triggers. The resulting application is not a fork in the software-engineering sense. It is a configuration layer that happens to live in the same repository shape as the substrate it depends on. New tables, new profiles, new routes, new triggers — all composed from the same engine.

This has already happened twice. The Wealth Manager described in Chapter 13 is one instance. A B2B growth and account-intelligence module — `stagent-growth` — is the second. Both are Stagent. Both are not Stagent. They are domain interpretations of a general-purpose substrate, and the substrate did not change to accommodate them.
...
```bash
# stagent-growth/.env.local
STAGENT_CLOUD_DISABLED=true
STAGENT_DATA_DIR=/Users/manavsehgal/.stagent-growth
```

The upstream repository ships with `STAGENT_DEV_MODE=true` and a `.git/stagent-dev-mode` sentinel file so the planned `instance-bootstrap` feature stays a no-op in contributor checkouts. When that feature ships, every private clone will automatically get a local branch, a pre-push hook that blocks accidental pushes to the upstream origin, and a stable `instanceId` recorded in its settings table. Today that automation is specified but not yet wired (`features/instance-bootstrap.md`); the convention — a `<domain>-mgr` branch plus a custom `STAGENT_DATA_DIR` — is already how every live clone operates. The Wealth Manager's `.env.local` points at `~/.stagent-wealth`. The Growth module's points at `~/.stagent-growth`. They run on the same laptop without collision.

**Layer 2: Project-scoped tables via idempotent bootstrap.** The Wealth Manager and the Growth module took different routes to creating their domain tables, and the contrast is instructive. The Wealth Manager reads from tables that are pre-seeded as Stagent templates — positions, transactions, watchlist, alerts — and then adds its own TypeScript interfaces over the shared `userTableRows` storage. The Growth module bootstraps its tables on demand, the first time a project opens the `/growth` surface:

```typescript
// stagent-growth/src/lib/growth/bootstrap.ts
...
```yaml
# stagent-wealth/src/lib/agents/profiles/builtins/wealth-manager/profile.yaml
id: wealth-manager
name: Wealth Manager
domain: personal
...
```typescript
// stagent-growth/src/lib/growth/profiles.ts
...
There is a phrase in infrastructure engineering: "cattle, not pets." Servers should be interchangeable units, not unique snowflakes. The equivalent phrase for domain applications on Stagent is "configuration over code."
...
A Stagent domain application is configuration. It has table template references, profile definitions, blueprint YAML files, trigger rows, and route components. Maintaining it means updating the definitions. Scaling it means running the same Stagent instance with more data. Each application is cattle — one instance of a pattern, deployable from a template, replaceable by re-running the setup.
...
The most interesting property of this pattern is that the tool used to build domain applications IS a domain application. Stagent is itself a configuration of its own primitives, pointed at the domain of "agent workspace management."

When you clone Stagent and prompt it to build a Growth module, you are using a domain application (agent workspace) to build a domain application (Growth). The profiles that assist in the build — general, code-reviewer, document-writer — are the same kind of artifact as the profiles being created. The workflow blueprints that orchestrate the build process are the same kind of artifact as the blueprints being produced.

```
stagent/ (the substrate)
  └── prompt or UI interaction
       └── stagent-wealth/ (domain app #1, branch wealth-mgr)
       └── stagent-growth/ (domain app #2, branch growth-mgr)
       └── stagent-{domain-N}/ (future)
```

Each arrow in that tree is the same operation: use Stagent's primitives to define a domain's tables, profiles, blueprints, triggers, and views. The operation is repeatable because the primitives are stable. The operation is fast because the primitives are expressive. The operation produces distributable output because the primitives are declarative.
