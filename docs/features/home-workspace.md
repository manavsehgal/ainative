...
Items requiring human action -- such as permission requests, failed tasks, stalled workflows, or agent handoff approvals -- surface here automatically. This ensures nothing slips through the cracks when agents need your input.
...
### Workspace Indicator
The sidebar footer shows the active workspace name and path, so you always know which project context agents are operating within.
