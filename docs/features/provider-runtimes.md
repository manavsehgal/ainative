...
- Two auth modes for Codex App Server:
  - **ChatGPT** -- browser sign-in tied to your ChatGPT plan, with cached session reuse in `ainative-business`'s isolated Codex home
...
- **`hasNativeSkills`** / **`stagentInjectsSkills`** / **`autoLoadsInstructions`** — decide whether `ainative-business` should inject SKILL.md or trust the runtime's native loader, preventing duplicated context on Codex and Claude.

Reading this matrix before wiring a feature is the canonical way to answer "should `ainative-business` do X, or trust the runtime to do X."
